...
class BaseGroupMembershipBackend(object):
    ...
    def set_memberships_from_ldap(self, user, membership):
        ...
        """
        ldap_groups = [l_grp for l_grp in LDAPGroup.objects.all() if \
            any([l_grp.org_unit in m for m in membership])]
        for l_grp in ldap_groups:
            for grp in l_grp.groups.all():
                user.groups.add(grp)
                
        if any([l_grp.make_staff for l_grp in ldap_groups]):
            user.is_staff = True
            
        if any([l_grp.make_superuser for l_grp in ldap_groups]):
...
class ActiveDirectoryGroupMembershipSSLBackend(BaseGroupMembershipBackend):
    ...
    def bind_ldap(self, username, password):
        try:
            # The ldap_groups webapp views use NT4_DOMAIN, so we leave this
            binddn = "%s@%s" % (username, settings.NT4_DOMAIN)
            l = self.bind(binddn, password)
        except (ldap.INVALID_CREDENTIALS, ldap.REFERRAL), e:
            # try subdomains
            for index, subdomain in enumerate(settings.NT4_SUBDOMAINS):
                try:
                    binddn = "%s@%s" % (username, subdomain + settings.NT4_DOMAIN)
                    l = self.bind(binddn, password)
                    break  # successful bind so return
                except (ldap.INVALID_CREDENTIALS, ldap.REFERRAL), e:
                    if index < len(settings.NT4_SUBDOMAINS) - 1:
                        continue  # try subsequent subdomains
                    else:
                        raise e  # raise if last subdomain failed
        return l
