...
def ajax_showresults(request,testplan_id, testcase_id):
    results = Result.objects.filter(testplan_testcase_link__testplan_id = testplan_id,testplan_testcase_link__testcase_id = testcase_id).order_by('-timestamp')
...
def ajax_fetch_designsteps(request, testcase_id):
    ...
    return HttpResponse(json.dumps({"data":list(design_steps), "currpage":1, "totalpages":1, "totalrecords":len(design_steps)}))
...
def ajax_fetch_uploads(request, testcase_id):
    ...
    return HttpResponse(json.dumps({"data":list(uploads), "currpage":1, "totalpages":1, "totalrecords":len(uploads)}))
...
def ajax_addtests(request, testplan_id):
    ...
    try:
        ...
        for testcase_id in add_list:
            testplan_testcase_link = TestplanTestcaseLink(testplan=testplan, testcase=TestCase.objects.get(pk=testcase_id))
        ...
        if actual_count == expected_count:
            ...
            message = "Added %d tests to <em>%s<em>.\n%d tests already present." % (net_count, testplan.name, present_count)
        return HttpResponse(
            json.dumps({
...
def ajax_removetests(request, testplan_id):
    ...
    try:
        ...
        if actual_count == expected_count:
            ...
            present_count = received_count - net_count
            message = "Deleted %d tests to <em>%s<em>.\n%d tests already deleted." % (net_count, testplan.name, present_count)
        return HttpResponse(
            json.dumps({
                'currentEdit': current_edit,
                "alertType": alert_type,
...
def ajax_tests(request):
    ...
    if 'search' in request.GET and request.GET['search'] == "true":
        ...
        if request.GET.get('asproduct'):
            tests = tests.filter(folder_path__startswith="Subject/" + request.GET['asproduct'].replace("+"," "))
...
def ajax_deletefolder(request):
    ...
    if count_of_tests_in_folder > 0:
        resp["deleted"] = False
        resp["message"] = "Folder " + str(folder.name) + " has " + str(count_of_tests_in_folder) + " test cases. Move these test cases before deleting the folder."	
    elif has_subfolder > 0:
        resp["deleted"] = False
        resp["message"] = "Folder " + str(folder.name) + " has a child folder. Delete this the child folder before deleting the folder."	
    else:
        resp["deleted"] = True
